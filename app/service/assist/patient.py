...
def _sanitize_response(patients_raw_list: list) -> list:
    result = []

    for patient in patients_raw_list:
        ...
        record = PatientFoundItem(
            ...
            person_card_id=(
                str(patient.get("PersonCard_id"))
                if patient.get("PersonCard_id")
                else None
            ),
...
async def fetch_patients_list(gateway_service: GatewayService, payload: PatientRequest):
    ...
    params = {"c": "Person6E", "m": "getPersonGrid", "_dc": int(time.time())}
