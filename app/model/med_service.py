...
class MedServiceListResponse(BaseModel):
    ...
    group_name: str = Field(validation_alias="MedService_Name")
    group_id: str = Field(validation_alias="MedService_id")


class ResearchListItemResponse(BaseModel):
    """Модель для ответа на запрос списка услуг в группе"""

    usluga_complex_name: Optional[str] = Field(
        None, validation_alias="UslugaComplex_Name"
    )
    usluga_complex_code: Optional[str] = Field(
        None, validation_alias="UslugaComplex_Code"
    )
    ...
    usluga_complex_resource_id: Optional[str] = Field(
        None, validation_alias="UslugaComplexResource_id"
    )
